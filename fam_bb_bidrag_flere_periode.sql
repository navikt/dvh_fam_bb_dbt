...
{% for period in periods %}
    ...
    SELECT /*+ parallel(64) */
        ...
        case 
            when valutakode != 'NOK' then 
                belop * (
                    select valutakurser
                    from dim_nb_valuta nb
                    where nb.base_cur = valutakode
                      and nb.periode <= aar_maaned
    ...
    FROM (
        ...
        opphor_fra as (
              ...
              from fam_bb_fagsak_ord fagsak 
        
              join fam_bb_bidrags_periode_ord periode
              on fagsak.pk_bb_fagsak = periode.fk_bb_fagsak
        ...
        fagsak_inntekt as (
          select
            ...
            f.vedtakstidspunkt,
            f.saksnr, f.fk_person1_kravhaver--, f.stonadstype		
        
          from fagsak_fylt_verdier f
      ...
      LEFT JOIN (
          SELECT * FROM (
              SELECT i.*,
                    ROW_NUMBER() OVER (PARTITION BY i.saksnr, i.fk_person1_kravhaver 
                    ORDER BY 
